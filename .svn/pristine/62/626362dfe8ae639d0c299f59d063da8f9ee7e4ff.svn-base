...
public class SchemaCheckDAO {
	 ...
	 * @throws InvalidKeyException
	 */
	public HashMap<String, String> compareSchema(String path,
			RequestDetails bean) throws FRDMException, IOException {
		...
		try {
			...
			try {
				...
				util.writeToOutPutStream(path, new Date()
						+ " INFO: Hive schema: " + destinationMap + "\n");
			...
			if (convSourceMap.equals(destinationMap)) {
				...
				util.writeToOutPutStream(
						path,
						new Date()
								+ " INFO: No Mismatch between source and destination schemas.\n");
				...
				if (!colAddMismatch.isEmpty() && colDelMismatch.isEmpty()
					...
					util.writeToOutPutStream(path,
							new Date() + " INFO: Columns Missing in Hive"
									+ addCols.substring(1) + "\n");
					...
					util.writeToOutPutStream(path, new Date()
							+ " INFO: Columns added to Hive Successfully \n");
					...
					if (!colDelMismatch.isEmpty()) {
						...
						util.writeToOutPutStream(path, new Date()
								+ " INFO: Columns Missing from Source"
								+ delCols.substring(1) + "\n");
					}
					if (!dtMismatch.isEmpty()) {
						...
						util.writeToOutPutStream(path, new Date()
								+ " INFO: Datatype Mismatched columns:"
								+ mismatchString.substring(1) + "\n");
					}
				}
			}
		...
		if (existflag) {
			util.writeToOutPutStream(
					...
					new Date()
							+ " INFO: Table "
							+ table
							+ " exist in Hive. Comparing Source and destination schemas"
							+ "\n");
			HashMap<String, String> mismatchMap = compareSchema(path, request);
			...
			if (mismatchtable.isEmpty()) {
				...
				requestLogDetails.setTargetdirectory(tarDir);
				util.writeToOutPutStream(path, new Date()
						+ " INFO: Creating new hive temp table "
						+ mismatchtable + "\n");
				new HiveDAO().createHiveTable(request, mismatchtable);
				util.writeToOutPutStream(path, new Date()
						+ " INFO: Temp table " + mismatchtable
			...
			requestLogDetails.setTargetdirectory(tarDir);
			util.writeToOutPutStream(path, new Date()
					+ " INFO: Creating new hive table " + table + "\n");
			new HiveDAO().createHiveTable(request, table);
			util.writeToOutPutStream(path, new Date() + " INFO: Hive table "
